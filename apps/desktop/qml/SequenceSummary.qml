...
Rectangle {
    ...
    // R6.4: Sequence is a pure consumer of the immutable fundamental frame
    // owned by PhasorView. It never falls back to a global cursor or launches a
    // second request, so C1/C2 cannot compete for duplicate DFT work.
    readonly property var effectiveSnapshot: snapshot && snapshot.valid
